...
TEST(EventBusTest, SubscribeAndPublish) {
    ...
    bus.subscribe("TestEvent", [&callback_count](const Event& e) {
        callback_count++;
    });
...
TEST(EventBusTest, CanvasModifiedEvent) {
    ...
    bus.subscribe("CanvasModified", [&layer_id_received](const Event& e) {
...
TEST(EventBusTest, ToolSelectedEvent) {
    ...
    bus.subscribe("ToolSelected", [&tool_id_received](const Event& e) {
...
TEST(WindowManagerTest, GetEventBus) {
    WindowManager wm;
    EventBus& bus = wm.get_event_bus();
    
    // Subscribe to event bus from window manager
    int callback_count = 0;
    bus.subscribe("TestEvent", [&callback_count](const Event& e) {
        callback_count++;
    });
